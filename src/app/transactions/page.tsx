...
function RecurringEntry({ cats, rules, reload }: { cats: Cat[]; rules: any[]; reload: () => Promise<void> }) {
  ...
  return <div className="card wide recurring-entry"><h2>{editing?'Edit Recurring':'Add Recurring'}</h2>{error&&<p className="error">{error}</p>}<div className="recurring-fields"><div><label>Category</label><span className="plus-field"><select value={cat} onChange={e=>{setCat(e.target.value);setSub('')}}><option value="">Category</option>{cats.filter(c=>c.direction==='expense').map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select><button type="button" onClick={addCat}>+</button></span></div><div><label>Subcategory</label><span className="plus-field"><select value={sub} disabled={!cat} onChange={e=>setSub(e.target.value)}><option value="">None</option>{subs.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select><button type="button" disabled={!cat} onClick={addSub}>+</button></span></div><div><label>Frequency</label><select value={freq} onChange={e=>setFreq(e.target.value)}><option>daily</option><option>weekly</option><option>bi-weekly</option><option>monthly</option><option>yearly</option></select></div><div><label>Amount</label><input type="number" step="0.01" value={amount} onChange={e=>setAmount(e.target.value)}/></div><div><label>Merchant</label><input value={merchant} onChange={e=>setMerchant(e.target.value)}/></div><div><label>Description</label><input value={note} onChange={e=>setNote(e.target.value)}/></div><div><label>Start date</label><input type="date" value={start} onChange={e=>setStart(e.target.value)}/></div><div><label>End date</label><input type="date" value={end} onChange={e=>setEnd(e.target.value)}/></div></div><button className="btn" style={{width:'auto'}} onClick={save}>{editing?'Save':'Add'}</button>{editing&&<button className="btn secondary" style={{width:'auto',marginLeft:8}} onClick={()=>setEditing(null)}>Cancel</button>}<h3>Current recurring expenses</h3>{rules.length === 0 && <p className="muted">No recurring expenses found.</p>}{rules.map(r=><div className="rule-row" key={r.id}>
