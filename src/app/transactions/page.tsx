...
function RecurringEntry({ cats, rules, reload }: { cats: Cat[]; rules: any[]; reload: () => Promise<void> }) {
  ...
  async function save() {
    setError('');
    if (!cat || !amount) { setError('Category and amount required'); return; }
    try {
      const r = await fetch('/api/recurring-rules' + (editing ? '?id=' + editing.id : ''), {
        method: editing ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ categoryId: cat, subcategoryId: sub || null, frequency: freq, amount, merchant, note, anchorDate: start || undefined, endDate: end || null }),
      });
      const d = await r.json().catch(() => ({}));
      if (!r.ok) { setError(d.error || `Save failed (${r.status})`); return; }
      if (!d.rule?.id) { setError('Save failed: server returned no rule'); return; }
      setEditing(null); setAmount(''); setMerchant(''); setNote(''); setStart(''); setEnd(''); await reload();
  ...
  return <div className="card wide recurring-entry"><div className="recurring-content"><h2>{editing?'Edit Recurring':'Add Recurring'}</h2>{error&&<p className="error">{error}</p>}<div className="recurring-fields"><div><label>Category</label><span className="plus-field"><select value={cat} onChange={e=>{setCat(e.target.value);setSub('')}}><option value="">Category</option>{cats.filter(c=>c.direction==='expense').map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select><button type="button" onClick={addCat}>+</button></span></div><div><label>Subcategory</label><span className="plus-field"><select value={sub} disabled={!cat} onChange={e=>setSub(e.target.value)}><option value="">None</option>{subs.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select><button type="button" disabled={!cat} onClick={addSub}>+</button></span></div><div><label>Frequency</label><select value={freq} onChange={e=>setFreq(e.target.value)}><option>daily</option><option>weekly</option><option>bi-weekly</option><option>monthly</option><option>yearly</option></select></div><div><label>Amount</label><input type="number" step="0.01" value={amount} onChange={e=>setAmount(e.target.value)}/></div><div><label>Merchant</label><input value={merchant} onChange={e=>setMerchant(e.target.value)}/></div><div><label>Description</label><input value={note} onChange={e=>setNote(e.target.value)}/></div><div><label>Start date</label><input type="date" value={start} onChange={e=>setStart(e.target.value)}/></div><div><label>End date</label><input type="date" value={end} onChange={e=>setEnd(e.target.value)}/></div></div><button className="btn" style={{width:'auto'}} onClick={save}>{editing?'Save':'Add'}</button>{editing&&<button className="btn secondary" style={{width:'auto',marginLeft:8}} onClick={()=>setEditing(null)}>Cancel</button>}<h3>Current recurring expenses</h3>{rules.length === 0 && <p className="muted">No recurring expenses found.</p>}{rules.map(r=><div className="rule-row" key={r.id}>
          <div className="rule-main"><strong>{r.merchant||'(no merchant)'}</strong><span>Amount: ${(r.amount / 100).toFixed(2)}</span><span className="rule-freq">{r.frequency}</span><span className="rule-enddate">End date: {r.endDate || '—'}</span></div>
          <div className="rule-actions"><span className="rule-desc">Description: {r.note || '—'}</span><button className="btn secondary" onClick={()=>edit(r)}>Edit</button><button className="btn secondary" onClick={async()=>{if(confirm('Delete this recurring rule?')){if(editing&&editing.id===r.id){setEditing(null);setCat('');setSub('');setFreq('monthly');setAmount('');setMerchant('');setNote('');setStart('');setEnd('')}await fetch('/api/recurring-rules?id='+r.id,{method:'DELETE'});await reload()}}}>Delete</button></div>
        </div>)}</div></div>
}
