...
export default function Reports() {
  ...
  function exportCsv() {
    ...
    for (const c of data.byCategory) rows.push(`${c.category},${(c.amount / 100).toFixed(2)}`);
    rows.push('');
    ...
    for (const c of data.yearlyByCategory) rows.push(`${c.category},${(c.amount / 100).toFixed(2)}`);
    rows.push('');
